...
**The page links; the claim does not (Ryan, 2026-09-13).** Ruling Four is about the claim, and it stands. The club-season page derives the link at read time from the man's own staff claim for the same line — printed name, year, club code and source — and labels it `person_basis: derived`, with how he was joined. One match links; none says why (`person_why`); two is refused; nothing is stored. The served sentence is reworded for a linked entry; the stored claim's text is not. **Captain-or-coach lines do not link:** several print two or more men in one line and no person carries a claim for any of them, so a link would be a new join made by the page. Gate: `gate_staff_people` S6, which failed on 26 unlinked entries and 9 unexplained ones before the change. Bios and counts for staff-only men wait for the media-guide front offices.
